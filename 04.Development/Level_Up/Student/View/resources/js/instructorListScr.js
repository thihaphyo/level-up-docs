...
let instrCount = instrList.length;
...
function buildInstructorList() {
    ...
    for (let i = 0; i < pages; i++) {
        $('#i-list-pagination').append(
            `<li><a class="pagination-link i-pagination-link" aria-label="Page ${i + 1}" aria-current="page">${i + 1}</a></li>`);
    }
...
function changePage(nextIndex) {
    ...
    document.querySelectorAll('.i-pagination-link')[curPageIndex].classList.remove('is-current');
    curPageIndex = nextIndex;
    document.querySelectorAll('.i-pagination-link')[curPageIndex].classList.add('is-current');
    ...
    for (let i = index, limit = index + paginationLimit; i < limit; i++) {
        ...
        $('.i-instructor-list').append(
            ...
            "<div class='i-instructor-list-img-wrapper'><img src='../View/" + instrList[i]['profile_image'] + "' alt='" + instrList[i]['full_name'] + "'/><div class='i-list-wrapper1'></div></div>" +
            "<div class='i-list-content'>" +
            "<div><p class='i-list-name'>" + instrList[i]['full_name'] + "</p>" +
            "<div class='i-list-des'><img src='../View/resources/img/icons/i_jobpos.svg' /><p class='i-list-pos'>" + instrList[i]['job_position'] + "</p></div>" +
            "<div class='i-list-des'><img src='../View/resources/img/icons/i_coursecount.svg' /><p class='i-list-courses'> Teaching " + instrList[i]['course_count'] + " course" + (instrList[i]['course_count'] > 1 ? 's' : '') + "</p></div></div>" +
