...
let csvFolderURL =
  "http://localhost/level-up-docs_original/04.Development/Level_Up/Instructor/Controller/orderlistDownloads/";
...
if (orderList.length === 0) {
  ...
  $(".o-table").empty();
  $(".o-table").append("<p class='o-no-order'>No order yet.</p>");
...
function buildorderList() {
  ...
  for (let i = 0; i < pages; i++) {
    $("#o-list-pagination").append(
      `<li><a class="pagination-link o-pagination-link" aria-label="Page ${
        i + 1
...
function showChangedPage(nextIndex, data) {
  ...
  document
    .querySelectorAll(".o-pagination-link")
...
$("#o-download-list").click((e) => {
  $.post(controllerURL, { downloadAll: instructorId }, (data) => {
    /**
